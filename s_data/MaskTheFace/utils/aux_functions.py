...
import dlib
from s_data.MaskTheFace.utils.read_cfg import read_cfg
from s_data.MaskTheFace.utils.fit_ellipse import *
import random
from s_data.MaskTheFace.utils.create_mask import texture_the_mask, color_the_mask
...
def mask_face2(image, six_points, angle, type="surgical", pattern=None, color=None):
    # Find the face angle
    threshold = 13
    if angle < -threshold:
        type += "_right"
    ...
    w = image.shape[0]
    h = image.shape[1]
    cfg = read_cfg(config_filename="s_data/MaskTheFace/masks/masks.cfg", mask_type=type, verbose=False)
    img = cv2.imread(cfg.template, cv2.IMREAD_UNCHANGED)

    # Process the mask if necessary
    if pattern:
        # Apply pattern to mask
        img = texture_the_mask(img, pattern, random.random())

    if color:
        # Apply color to mask
        img = color_the_mask(img, color, random.random())
...
import time
def mask_image2(dlib_detector, dlib_predictor, image, mask_type='N95', pattern=None, color=None):
        face_locations = dlib_detector(image, 1)
        ...
        if len(face_locations) == 0:
            ...
            shape = dlib_predictor(image, face_location)
...
def get_available_mask_types(config_filename="s_data/MaskTheFace/masks/masks.cfg"):
    parser = ConfigParser()
